...
import {Animated, EmitterSubscription, Keyboard, KeyboardEvent, Text, TouchableOpacity, View,} from 'react-native'
...
class ToastManager extends Component<ToastManagerProps, ToastManagerState> {
    ...
    show = ({text, barColor, icon, duration, id, position, width}: ToastType) => {
    ...
    handleBar = () => {
        ...
        Animated.parallel(
            toasts.map((toast) => {
                ...
                // @ts-ignore
                animation.start(() => toast.barWidthAnimation._value === 0 && this.hideToast(toast.id))
                return animation
            }),
        )
    }
    ...
    renderToast = ({
        ...
        return (
            <Modal
                key={key}
                animationIn={animationIn || animationStyleOptions[animationStyle!].animationIn}
                animationOut={animationOut || animationStyleOptions[animationStyle!].animationOut}
            ...
            >
                ...
                >
                    ...
                    <View style={styles.progressBarContainer}>
                        <Animated.View style={{width: barWidthAnimation, backgroundColor: barColor}}/>
                    </View>
                </View>
            </Modal>
        )
    ...
    render() {
        ...
        return toasts.map((toast, index) => {
            const positionOffset: number = toasts.reduce((reduceBottom, reduceToast, reduceIndex) => {
            ...
            return this.renderToast({
                ...toast,
                key: toast.id,
                positionOffset: this.props.position === "top" ? this.props.positionValue + positionOffset : positionOffset + this.props.positionValue + keyboardHeight,
            })
        })
    }
}
